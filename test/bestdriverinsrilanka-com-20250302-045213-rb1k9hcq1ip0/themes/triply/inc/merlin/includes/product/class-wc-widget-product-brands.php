 ...
 * @extends WC_Widget
 */
class Cinzal_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'triply');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'triply');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'triply'),
				'label' => esc_html__('Title', 'triply'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'triply'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'triply'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'triply'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'triply'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'triply'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'triply'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'triply'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Cinzal_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'triply');
			...
			echo '<ul class="product-brands" id="cinzal-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Cinzal_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'triply'),
		'singular_name'              => esc_html__('Brands', 'triply'),
		'menu_name'                  => esc_html__('Brands', 'triply'),
		'all_items'                  => esc_html__('All Brands', 'triply'),
		'parent_item'                => esc_html__('Parent Brand', 'triply'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'triply'),
		'new_item_name'              => esc_html__('New Brand Name', 'triply'),
		'add_new_item'               => esc_html__('Add New Brands', 'triply'),
		'edit_item'                  => esc_html__('Edit Brand', 'triply'),
		'update_item'                => esc_html__('Update Brand', 'triply'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'triply'),
		'search_items'               => esc_html__('Search Brands', 'triply'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'triply'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'triply'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
